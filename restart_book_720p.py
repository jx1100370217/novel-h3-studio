...
STATUS=REPO/'runtime/restart_book_720p/status.json'
TEST=REPO/'runtime/speech_root_cause/capacity_720p/status.json'
...
def main():
    ...
    while True:
        ...
        time.sleep(10)
    qc=result.get('qc',{})
    if not qc.get('passed') or qc.get('width')!=1280 or qc.get('height')!=720 or qc.get('frames')!=362:
    ...
    def retire(state):
        for take in state['takes'].values():
            take.update(status='removed_by_user',retired=True,video_deleted=True,deletion_reason='720p十五秒容量测试通过，按用户要求从头重拍')
    ...
    for chapter in chapters:
        ...
        approve_episode(ROOT,eid,'Codex 时长编排检查','用户授权720p从头重拍；保留锁定原文、说话人与参考资产，时长及逐秒时间线检查通过；视频仍需审片。')
