...
需要从其他电脑访问完整工作台时，运行 [`scripts/install_public_access.sh`](scripts/install_public_access.sh)。它会安装密码保护的公网网关，并在已登录 Tailscale 且已启用 Funnel 的情况下生成固定的 `*.ts.net` 地址；完整步骤见 [`docs/public-access.md`](docs/public-access.md)。不要把运行时凭证或项目文件提交到 Git。
